...
export class EKPRestClient {
  ...
  async getTodoList(
    ...
    page: number = 1,
    pageSize: number = 20
  ): Promise<EKPResponse<{ count: number; items: any[]; total: number }>> {
    try {
      // 计算起始位置
      const start = (page - 1) * pageSize;

      const response = await this.post<any>(EKP_REST_PATHS.notify.getTodo, {
        targets: JSON.stringify({ LoginName: loginName }),
        type: type,
        start: start,
        end: start + pageSize,
      ...
      if (response.result && response.result.returnState === 2) {
        try {
          ...
          return {
            success: true,
            data: { count: 0, total: 0, items: [] },
            msg: response.result.message || '获取待办列表成功',
          };
        }
      } else if (response.result && response.result.returnState === 1) {
        return {
  ...
  async approveTodo(
    ...
    loginName: string,
    opinion: string = '同意'
  ): Promise<EKPResponse<{ id: string; opinion: string }>> {
    try {
      const response = await this.post<any>(EKP_REST_PATHS.notify.getTodo, {
        targets: JSON.stringify({ LoginName: loginName }),
        type: 1, // 审批类
      ...
      if (response.result && response.result.returnState === 2) {
        ...
        return {
          success: false,
          data: null,
          msg: `审批失败：${response.result.message}`,
        };
      }
      ...
      return {
        success: response.status === 200,
        data: { id: todoId, opinion },
        msg: response.status === 200 ? '审批成功' : `审批失败：HTTP ${response.status}`,
      };
    } catch (err) {
      return {
        success: false,
        data: null,
        msg: `审批失败：${err instanceof Error ? err.message : '网络错误'}`,
      };
    }
  }
