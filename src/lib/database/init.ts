 ...
 * 检查数据库表是否已初始化
 */
async function isTablesInitialized(): Promise<boolean> {
  try {
 ...
 * 初始化组织架构表
 */
async function initOrganizationTables(): Promise<void> {
  const tables = [
    // 组织元素表（机构/部门/岗位）
    `
  ...
  for (const sql of tables) {
    try {
      ...
      console.log('[AutoInit] 表创建成功');
    } catch (error: unknown) {
      console.error('[AutoInit] 表创建失败:', error);
      throw error;
    }
  }
  ...
  try {
    ...
    console.error('[AutoInit] 初始化数据失败:', error);
    // 不抛出错误，因为可能数据已经存在
  }
}
...
export async function autoInitializeDatabase(): Promise<void> {
  try {
    ...
    if (!dbManager.isConnected()) {
      console.log('[AutoInit] 数据库未连接，跳过自动初始化');
      return;
    }

    // 检查表是否已初始化
    const isInitialized = await isTablesInitialized();

    if (isInitialized) {
      console.log('[AutoInit] 数据库表已存在，跳过自动初始化');
      return;
    }

    console.log('[AutoInit] 开始自动初始化数据库表...');
    await initOrganizationTables();
    console.log('[AutoInit] 数据库自动初始化完成');
  } catch (error) {
    console.error('[AutoInit] 数据库自动初始化失败:', error);
    // 不抛出错误，避免影响应用启动
  }
}
